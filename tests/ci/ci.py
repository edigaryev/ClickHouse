...
class CiCache:
    ...
    def __init__(
        ...
        self.s3 = s3
        self.build_digest = job_digests[Build.PACKAGE_RELEASE]
    ...
    def _get_job_status_file_name_and_s3path(
        ...
        if CI_CONFIG.is_docs_job(job_name):
    ...
    def fetch_job_statuses(self):
        ...
        # rename ".master.ci" records to regular ".ci" as
        #   we passed that info to self.successful_on_master_jobs already and want all files have identical naming
        for file in self.cache_path.iterdir():
            if file.is_file() and file.name.endswith(".master.ci"):
                file.rename(Path(file.parent) / file.name.replace(".master.ci", ".ci"))
    ...
    def push_pending(
        ...
        for batch in batches:
            ...
            pending_state = PendingState(
                updated_at=time.time(), run_url=GITHUB_RUN_URL, state=state
            )
            with open(file_name, "w") as json_file:
                json.dump(asdict(pending_state), json_file)  # type: ignore
            _ = self.s3.upload_file(
...
def _mark_success_action(
    ...
    if job_config.run_always or job_config.run_by_label:
        ...
        if pr_info.is_master():
            ci_cache.delete_pending(job, batch, num_batches)
        if job_status and job_status.is_ok():
            ci_cache.push(job, batch, num_batches, job_status, pr_info.is_master())
...
def _configure_jobs(
    ...
    for job in digests:
        ...
        for batch in range(num_batches):  # type: ignore
            if job_config.disabled_on_master and PRInfo().is_master():
                continue
            if job_config.run_by_label and job_config.run_by_label in pr_labels:
                ...
                # always add to todo
                batches_to_do.append(batch)
            elif rebuild_all_binaries and CI_CONFIG.is_build_job(job):
                batches_to_do.append(batch)
            elif not ci_cache:
                batches_to_do.append(batch)
                ...
                if ci_cache.is_pending(job, batch, num_batches):
